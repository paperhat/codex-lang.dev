...
The bootstrap schemas (`bootstrap-schema/schema.cdx` and `bootstrap-schema/simplified/schema.cdx`) instantiate the schema-definition rules defined in this specification. They are derived artifacts, not independent sources of authority. Appendix C enumerates every construct the bootstrap schema-of-schemas defines; §11 remains the semantic authority for what each construct means, and Appendix C is the exhaustive authority for what constructs exist.

If an implementer discovers an apparent conflict between any parts of this specification, between this specification and the bootstrap schemas, or between §11 and Appendix C, that conflict is a defect. Implementers MUST NOT guess which source prevails. Implementers MUST report the defect.
...
- If the file starts with `00 00 FE FF` (4 bytes), reject with a `ParseError` (§14): UTF-32 is not supported.
- If the file starts with `FF FE 00 00` (4 bytes), reject with a `ParseError` (§14): UTF-32 is not supported.
- If the file starts with `FE FF` (2 bytes), interpret the file as UTF-16 Big Endian.
- If the file starts with `FF FE` (2 bytes), interpret the file as UTF-16 Little Endian.
...
| max | $FiniteRealNumber | — | No | §11.3.4 |
| maxCount | $NonNegativeInteger | — | No | §11.8.2 |
| message | $Text | — | No | §9.5.2 |
| min | $FiniteRealNumber | — | No | §11.3.4 |
...
The bootstrap schema defines the following 26 ConstraintDefinitions. These enforce structural well-formedness rules on the schema-definition concepts themselves.
...
| EnumeratedValueSet/builtInRedefinitionProhibition | EnumeratedValueSet | name must not be one of the five built-in set names (Cardinality, CompatibilityClass, ConceptKind, EntityEligibility, Ordering) | §11.5.4 |
...
The bootstrap schema defines the following 5 ValueTypeDefinitions.

| Name | Base Value Type | Validator | Spec Reference |
|------|----------------|-----------|----------------|
| ConceptName | $Text | — | §11.5.2 |
| LookupToken | $LookupToken | — | §11.5.2 |
...
| ConstraintDefinitions | 26 |
| ValueTypeDefinitions | 5 |
