...
- [x] Colors (Batches 2–7: semantics + conversions + conformance) — Completed schema-directed semantic IR for all color spellings, specified a single deterministic XYZ D65 interchange conversion graph for all `color(...)` spaces (including transfer functions, matrices, and D50/D65 adaptation), fully defined reject-only evaluation for `color-mix(...)` and all `from <color>` relative forms, and made named colors fully deterministic by defining fixed sRGB hex expansions (including `&transparent`). Added conformance fixtures for the new semantic-invalid rules and verified repo checks are green.
